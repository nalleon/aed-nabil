                ...
                <div class="row mt-5 justify-content-center w-100">
                    <ul class="list-unstyled w-100 px-5">
                        ...
                        <li class="d-flex justify-content-between align-items-center mx-auto px-4 py-3 mb-3 border rounded w-100">
                            <a href="{{ route('edituser', ['id' => $user->getId()] )}}"class="link-offset-1 link-underline link-underline-opacity-0 link-light fw-bold fs-4">
                                {{ $user->getName() }}
                            </a>
                            <form action="{{ route('deleteuser', ['id' => $user->getId()]) }}" method="POST" class="mb-0">
                                @csrf
